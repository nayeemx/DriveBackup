...
## 21. Backup fails on Windows-illegal characters and duplicate filenames in Google Drive

| | |
|---|---|
| Status | **SOLVED** (v0.1.44) |
| Symptom | Backup starts but rclone fails with errors like `Failed to copy: file name not valid on Windows` for files containing `:`, `*`, `Ø`, or other characters illegal on NTFS. Also, duplicate filenames in the same Drive folder (e.g. two `Book2.xlsx`) cause rclone to fail with `ERROR : ... corrupted on transfer: sizes differ`. |
| Root cause | Google Drive allows any filename characters, but Windows NTFS forbids `: * ? " < > |`. rclone's default local encoding doesn't translate these. For duplicates, rclone's `--backup-dir` wasn't configured, so the second file overwrote the first and then `check` detected the mismatch. |
| Fix | Three-part fix across v0.1.42–v0.1.44: 1) **v0.1.42**: Added `--local-encoding` flag (`Colon,Asterisk,Question,DoubleQuote,LtGt,Pipe,Ctl`) to rclone `copy()` and `check()` in `rclone_manager.py`. Fixed temp file dir to use `tempfile.gettempdir()` instead of `local_dir.parent`. 2) **v0.1.43**: Added `--backup-dir` (`local_dir.parent / "_backup_conflicts"`) for duplicate filename handling — rclone moves the conflicting file to the backup dir instead of failing. 3) **v0.1.44**: Smart download strategy in `backup.py` — new `_scan_inventory()` function scans all Drive files before downloading, categorizes them as safe (no illegal chars, unique names) or problematic. Safe files download first in bulk with retry. Problematic files are attempted one-by-one with per-file error handling. Failures logged to `state/failed_files.json`. GUI shows skip count in completion summary. Backup never fails completely — always saves what it can. |
| Verification | Smoke test passes all sections. Manual test: rclone copy with default encoding (no `--local-encoding` flag) downloads all 548 files including files with `:` and Unicode characters. |
| Notes | **Important finding**: rclone's default `--local-encoding` on Windows is `Slash,LtGt,DoubleQuote,Colon,Question,Asterisk,Pipe,BackSlash,Ctl,RightSpace,RightPeriod,InvalidUtf8,Dot` — it already handles all Windows-illegal characters. Our earlier explicit `--local-encoding Colon,Asterisk,Question,DoubleQuote,LtGt,Pipe,Ctl` was **stripping useful defaults** (Slash, BackSlash, RightPeriod), causing failures. Also fixed `_scan_inventory()` which was checking only filename (not full path) for duplicates — 396 `index.html` files in different folders were incorrectly flagged as "duplicate". |
