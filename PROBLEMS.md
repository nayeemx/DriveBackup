...
| Verification | Smoke test passes all sections. Manual: user's Google Drive (full 15GB, contains `:` and `Ø` filenames, duplicate `Book2.xlsx` and `cv.pdf`) — safe files download first, problematic files attempted individually, failures logged to `failed_files.json`. |
| Notes | `--local-encoding` valid values: `Colon,Asterisk,Question,DoubleQuote,LtGt,Pipe,Ctl`. `--backup-dir` must NOT be inside the destination directory (rclone error 7 if overlapping). |
